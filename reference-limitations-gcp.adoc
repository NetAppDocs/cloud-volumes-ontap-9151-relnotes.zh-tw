...
如果您 https://cloud.google.com/vpc/docs/private-service-connect["Google Private Service Connect"^]在部署 Cloud Volumes ONTAP 的 VPC 中使用，則需要實作 DNS 記錄，將流量轉送到所需的 https://docs.netapp.com/us-en/bluexp-setup-admin/task-quick-start-connector-google.html["BlueXP API端點"^]。
