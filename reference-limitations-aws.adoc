...
[role="lead"]
下列已知限制僅適用於Cloud Volumes ONTAP Amazon Web Services的功能。請務必同時檢閱link:reference-limitations.html["所有雲端供應商的不完整功能Cloud Volumes ONTAP"]。
...
Cloud Volumes ONTAP 在閒置時不會釋出 CPU 、因此 https://aws.amazon.com/cloudwatch/["Amazon CloudWatch"^]可以針對 EC2 執行個體回報高 CPU 警告、因為它會看到 100% 的使用率。您可以忽略此警報。使用「flex統計 資料」命令可顯示CPU的實際使用量。ONTAP
